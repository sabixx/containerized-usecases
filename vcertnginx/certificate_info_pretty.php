...
function get_certificate_data($cert_file) {
    ...
    if ($key_details['type'] === OPENSSL_KEYTYPE_RSA) {
        $data['Key Size'] = $key_details['bits'] . ' bits';
    } elseif ($key_details['type'] === OPENSSL_KEYTYPE_EC) {
        $data['Key Curve'] = $key_details['ec']['curve_name'];
    }    
...
// Retrieve headers using apache_request_headers() and get X-Certpath
$headers = apache_request_headers();
$cert_file = isset($headers['X-Certpath']) ? $headers['X-Certpath'] : '';  // Dynamically retrieve cert file path from header
